...
router.get('/orders', admin, async (c) => {
  const res = await c.env.DB.prepare('SELECT o.id, o.user_id, u.username, o.items, o.created_at FROM orders o JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC').all()
...
router.patch('/users/:id', admin, async (c) => {
  ...
  if (username) {
    const exists = await c.env.DB.prepare('SELECT id FROM users WHERE username = ? AND id != ?').bind(username, id).first()
...
router.post('/settings/r2', admin, async (c) => {
  ...
  // 检查是否已有配置
  const existing = await c.env.DB.prepare('SELECT id, access_key_id, secret_access_key FROM r2_config ORDER BY id DESC LIMIT 1').first<R2ConfigRow>()
